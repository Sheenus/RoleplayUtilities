...
public class GuiWhileUseEffectsList extends GuiListExtended {
	
	private final GuiWhileUseEffects whileUseEffectsGUI;
	private final Minecraft mc;
	private final IGuiListEntry[] listEntries;
	private int maxTextWidth;
	private final String description = I18n.format("reference.whileuseeffects.description");
	
	public GuiWhileUseEffectsList(GuiWhileUseEffects whileUse, Minecraft mcIn) {
		super(mcIn, whileUse.width, whileUse.height, Math.max(whileUse.height/5, 55), whileUse.height - whileUse.height/5, 15);
		this.whileUseEffectsGUI = whileUse;
		this.mc = mcIn;
		maxTextWidth = whileUse.width - ((whileUse.width - (getScrollBarX() - 10))*2);
	...
	public class textEntry implements IGuiListEntry {
		...
		public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
			mc.fontRenderer.drawString(this.text, whileUseEffectsGUI.width/2 - maxTextWidth/2, y, 16777215);
