...
public class GuiWeaponEffectsList extends GuiListExtended {
	
	private final GuiWeaponEffects WeaponEffectsGUI;
	private final Minecraft mc;
	private final IGuiListEntry[] listEntries;
	private int maxTextWidth;
	private final String description = I18n.format("reference.weaponeffects.description");
	
	public GuiWeaponEffectsList(GuiWeaponEffects weaponEffects, Minecraft mcIn) {
		super(mcIn, weaponEffects.width, weaponEffects.height, Math.max(weaponEffects.height/5, 55), weaponEffects.height - weaponEffects.height/5, 15);
		this.WeaponEffectsGUI = weaponEffects;
		this.mc = mcIn;
		maxTextWidth = weaponEffects.width - ((weaponEffects.width - (getScrollBarX() - 10))*2);
	...
	public class textEntry implements IGuiListEntry {
		...
		public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
			mc.fontRenderer.drawString(this.text, WeaponEffectsGUI.width/2 - maxTextWidth/2, y, 16777215);
