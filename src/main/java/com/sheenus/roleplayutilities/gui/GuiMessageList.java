...
public class GuiMessageList extends GuiListExtended {

	private final GuiMessage messageGUI;
	private final Minecraft mc;
	private final IGuiListEntry[] listEntries;
	private int maxTextWidth;
	private final String description = I18n.format("reference.message.description");
	
	public GuiMessageList(GuiMessage message, Minecraft mcIn) {
		super(mcIn, message.width, message.height, Math.max(message.height/5, 55), message.height - message.height/5, 15);
		this.messageGUI = message;
		this.mc = mcIn;
		maxTextWidth = message.width - ((message.width - (getScrollBarX() - 10))*2);
	...
	public class textEntry implements IGuiListEntry {
		...
		public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
			mc.fontRenderer.drawString(this.text, messageGUI.width/2 - maxTextWidth/2, y, 16777215); 
