...
public class GuiSetRedstoneActivateList extends GuiListExtended {
	
	private final GuiSetRedstoneActivate SetRedstoneActivateGUI;
	private final Minecraft mc;
	private final IGuiListEntry[] listEntries;
	private int maxTextWidth;
	private final String description = I18n.format("reference.setredstoneactivate.description");
	
	public GuiSetRedstoneActivateList(GuiSetRedstoneActivate SetRedstone, Minecraft mcIn) {
		super(mcIn, SetRedstone.width, SetRedstone.height, Math.max(SetRedstone.height/5, 55), SetRedstone.height - SetRedstone.height/5, 15);
		this.SetRedstoneActivateGUI = SetRedstone;
		this.mc = mcIn;
		maxTextWidth = SetRedstone.width - ((SetRedstone.width - (getScrollBarX() - 10))*2);
	...
	public class textEntry implements IGuiListEntry {
		...
		public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
			mc.fontRenderer.drawString(this.text, SetRedstoneActivateGUI.width/2 - maxTextWidth/2, y, 16777215);
