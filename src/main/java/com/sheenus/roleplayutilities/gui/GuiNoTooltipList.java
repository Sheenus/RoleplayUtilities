...
public class GuiNoTooltipList extends GuiListExtended {
	
	private final GuiNoTooltip noTooltipGUI;
	private final Minecraft mc;
	private final IGuiListEntry[] listEntries;
	private int maxTextWidth;
	private final String description = I18n.format("reference.notooltip.description");
	
	public GuiNoTooltipList(GuiNoTooltip noTooltip, Minecraft mcIn) {
		super(mcIn, noTooltip.width, noTooltip.height, Math.max(noTooltip.height/5, 55), noTooltip.height - noTooltip.height/5, 15);
		this.noTooltipGUI = noTooltip;
		this.mc = mcIn;
		maxTextWidth = noTooltip.width - ((noTooltip.width - (getScrollBarX() - 10))*2);
	...
	public class textEntry implements IGuiListEntry {
		...
		public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
			mc.fontRenderer.drawString(this.text, noTooltipGUI.width/2 - maxTextWidth/2, y, 16777215);
